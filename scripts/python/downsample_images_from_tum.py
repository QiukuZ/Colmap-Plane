...
if __name__ == '__main__':
    cam_path = '/home/alex/Desktop/Lidar_Visual_Plane/0430f2/data/raw_data/f2dongjian_2020-04-30-11-25-58'
    tum_path = '/home/alex/Desktop/Lidar_Visual_Plane/0430f2/data/raw_data/f2dongjian_2020-04-30-11-25-58.bag_tum.txt'
    out_path = "/home/alex/Desktop/Lidar_Visual_Plane/0430f2"
    cam = 'cam0'

    # 打开data.csv文件
    f_cam0 = open(cam_path + "/mav0/" + cam + "/data.csv", "r")
    ...
    # 保留双目同步的图像时间戳
    image_timestamp_num = 0
    image_timestamp = []
    for i in range(len(cam0_list)):
        image_timestamp_num = image_timestamp_num + 1
        image_timestamp.append(cam0_list[i])
    ...
    traj_last = []
    down_num = 0
    for i in range(image_timestamp_num):
        time_str = image_timestamp[i][0]
        ...
        if min_time < 0.05:
            ...
            elif is_big_move(traj_last, tum_list[min_tum_index], 4, 1.5, 0.5):
                traj_last = tum_list[min_tum_index]
                down_num = down_num + 1
                image_timestamp_traj.append([image_timestamp[i], tum_list[min_tum_index]])
    ...
    with open(out_path + "/geos.txt", 'w') as f_out:
        for i in range(len(image_timestamp_traj)):
            ...
            f_out.write( cam + '/' + f_str + ' ' + image_timestamp_traj[i][1][1] + ' ' + \
                    image_timestamp_traj[i][1][2] + ' ' + \
                    image_timestamp_traj[i][1][3] + '\n')
    for i in range(len(image_timestamp_traj)):
        im = cv2.imread(cam_path + "/mav0/" + cam + "/data/" + image_timestamp_traj[i][0][1])
        # cv2.imshow("im", im)
        # cv2.waitKey(1)
    print("down num = ", down_num)
    print("good num = ", good_tum_num)
    print("image num = ", image_timestamp_num)

    f = open(out_path + "/downsample_tum.txt", 'r')
    image_path = cam_path
    image_save_path = out_path + '/images_downsample'
    mkdir(image_save_path)
    mkdir(image_save_path+'/' + cam)
    lines = f.readlines()
    f.close()
    ...
    for i in range(len(lines)):
        ...
        image1 = cv2.imread(image_path + '/mav0/' + cam +'/data/' + time + '.png')
        if image1.shape[0] == 0 :
            print('Lost Frame ',time ,'.png')
            continue

        cv2.imwrite(image_save_path + '/'  + cam + '/' + time + '.png', image1)
